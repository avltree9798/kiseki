...
/* Note: TCC_TARGET_MACHO is currently cross-compile only (no -run support) */
#if defined _WIN32 == defined TCC_TARGET_PE && !defined TCC_TARGET_MACHO
...
/* ------------ tccmacho.c -------------- */
/*
 * Mach-O output support for ARM64 (aarch64) targets.
 * Currently supports executables and dynamic libraries (.dylib).
 * Limitations:
 *   - ARM64 only (no x86_64 Mach-O support yet)
 *   - No object file (.o) output (executables/dylibs only)
 *   - Cross-compile only (no -run support)
 */
