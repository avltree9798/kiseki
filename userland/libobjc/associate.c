...
// nsobject.h contains @interface — not needed in C, we use objc_msgSend directly
#include "spinlock.h"
#include "class.h"
#include "dtable.h"
#include "selector.h"
                         ...
                         uintptr_t policy)
{
	switch (policy)
	{
		...
		{
			// [(id)obj copy] -> objc_msgSend(obj, sel_registerName("copy"))
			static SEL copy_sel;
			if (!copy_sel) copy_sel = sel_registerName("copy");
			obj = ((id(*)(id, SEL))objc_msgSend)((id)obj, copy_sel);
			break;
		}
	...
	// Converted from @try/@finally — in the runtime itself,
	// we can safely do this without exception handling since
	// objc_release won't throw in our context.
	{
		if (OBJC_ASSOCIATION_ASSIGN != r->policy)
		{
			objc_release(r->object);
