...
#endif
// nsobject.h contains @interface — not needed, we use objc_msgSend directly
#include "class.h"
#include "selector.h"
// Override SELECTOR macro — replace @selector() with sel_registerName() for C++ compilation
#undef SELECTOR
 ...
 *
 * Converted from ObjC: @selector(selName) -> sel_registerName(#selName)
 */
#ifdef __GNUSTEP_MSGSEND__
#define ManualRetainReleaseMessage(object, selName, types) \
	((types)objc_msgSend)(object, sel_registerName(#selName))
#else
#define ManualRetainReleaseMessage(object, selName, types) \
	((types)(objc_msg_lookup(object, sel_registerName(#selName))))(object, sel_registerName(#selName))
#endif
...
// NSAutoreleasePool forward declaration — ObjC @interface converted to C++
// We send messages via objc_msgSend so no interface declaration is needed.
...
extern "C" OBJC_PUBLIC void objc_release_fast_np(id obj)
{
	...
	{
		((void(*)(id, SEL))objc_msgSend)(obj, sel_registerName("dealloc"));
	}
}
...
static inline void initAutorelease(void)
{
	...
	{
		AutoreleasePool = (Class)objc_getClass("NSAutoreleasePool");
		if (Nil == AutoreleasePool)
		{
		...
		else
		{
			...
			{
				// [AutoreleasePool class] -> force class initialisation
				((id(*)(id, SEL))objc_msgSend)((id)AutoreleasePool, sel_registerName("class"));
				NewAutoreleasePool = class_getMethodImplementation(object_getClass((id)AutoreleasePool),
				                                                   SELECTOR(new));
				DeleteAutoreleasePool = class_getMethodImplementation(AutoreleasePool,
				                                                      SELECTOR(release));
				AutoreleaseAdd = class_getMethodImplementation(object_getClass((id)AutoreleasePool),
				                                               SELECTOR(addObject:));
			}
		}
	}
...
static inline id autorelease(id obj)
{
	...
	if (objc_test_class_flag(classForObject(obj), objc_class_flag_fast_arc))
	{
		...
		if (0 != AutoreleaseAdd)
		{
			AutoreleaseAdd((id)AutoreleasePool, SELECTOR(addObject:), obj);
		}
		return obj;
	}
...
extern "C" OBJC_PUBLIC void *objc_autoreleasePoolPush(void)
{
	...
	return NewAutoreleasePool((id)AutoreleasePool, SELECTOR(new));
}
