...
void thread_sleep_ticks(uint64_t ticks)
{
     ...
     * IMPORTANT: Set TH_WAIT inside the lock to avoid a race where a
     * timer IRQ fires between setting TH_WAIT and adding to the sleep
     * queue — sched_switch would see TH_WAIT and not re-enqueue, but
     * the thread isn't in the sleep queue yet, so it would be lost. */
...
static struct thread *sched_steal_work(uint32_t my_cpu)
{
     ...
     * The run_count check above was done without the lock, so re-check
     * under the lock. This avoids unnecessary lock contention from idle
     * CPUs constantly taking other CPUs' run_lock in QEMU TCG. */
    ...
    /* Re-check without lock to avoid contention in the common case
     * (3 idle CPUs all trying to steal from CPU 0 every tick) */
    if (cd->run_count < 2)
        return NULL;
...
void sched_switch(void)
{
    ...
    if (new_thread == old) {
        /*
         * Same thread re-selected (only runnable thread on this CPU).
         * Reset quantum to avoid per-tick reschedule storm, and restore
         * effective_priority so a lone thread isn't permanently penalised.
         *
         * Without this, quantum stays at 0 (or negative) after the first
         * expiry, causing need_resched to fire EVERY tick (100x/sec) and
         * effective_priority to decay to PRI_MIN within < 1 second.
         */
        old->quantum = SCHED_QUANTUM_DEFAULT;
        if (old->effective_priority < old->priority)
            old->effective_priority = old->priority;
...
void sched_tick(void)
{
     ...
     * Also boost the currently-running thread — without this, a thread
     * that's the sole runnable thread never gets its priority restored
     * because it's never in the run queue when aging runs. */
    if ((cd->total_ticks % 100) == 0) {
        if (th->sched_policy == SCHED_OTHER && th->effective_priority < th->priority)
            th->effective_priority = th->priority;  /* full restore */
    }

    /* Priority aging for run-queue threads: every 100 ticks, boost starved threads */
    if ((cd->total_ticks % 100) == 0) {
