...
void trap_sync_el0(struct trap_frame *tf)
{
    ...
    case EC_DABT_LOWER: {
        ...
        /*
         * With T0SZ=16, valid TTBR0 (user) addresses must have bits [63:48]
         * all zero.  If any upper bit is set, the address is invalid and no
         * page table walk can resolve it — deliver SIGSEGV immediately.
         * This prevents vmm_translate from silently truncating the VA via
         * index masking and finding unrelated kernel pages.
         */
        if (fault_va >> 48) {
            /* Bits [63:48] non-zero — invalid user address */
            break;  /* fall through to SIGSEGV */
        }
        ...
        if (fault_type == 0x04) {
            ...
            if (p && p->p_vmspace && p->p_vmspace->pgd) {
                ...
                if (page_va < 0xFFFF000000000000ULL && page_va != 0) {
                    /*
                     * Bug 17b: Check if PTE already exists before demand-paging.
                     * If a valid PTE exists, the translation fault was spurious
                     * (stale TLB). Just do a TLBI and retry — do NOT allocate
                     * a new zero page, as that would overwrite existing data.
                     */
                    uint64_t existing_pa = vmm_translate(p->p_vmspace->pgd, page_va);
                    if (existing_pa != 0) {
                        /*
                         * PTE exists but we got a translation fault — could
                         * be stale TLB.  Try TLBI once; if same VA faults
                         * again the problem is structural → deliver SIGSEGV.
                         */
                        if (p->p_last_fault_va == page_va) {
                            /* Second fault on same VA — TLBI didn't help */
                            kprintf("[demand-page] VA 0x%lx still faulting after TLBI! "
                                    "pid=%d PC=0x%lx — delivering SIGSEGV\n",
                                    page_va, p->p_pid, tf->elr);
                            p->p_last_fault_va = 0;
                            /* Fall through to SIGSEGV below */
                        } else {
                            /* First time — try TLBI and retry */
                            p->p_last_fault_va = page_va;
                            __asm__ volatile(
                                "tlbi vale1is, %0; dsb ish; isb"
                                :: "r"(page_va >> 12)
                                : "memory"
                        ...
                        /*
                         * Genuine missing page — demand-page a zero page.
                         *
                         * This handles:
                         *   - Stack growth (mapped by execve without vm_map entries)
                         *   - BSS/heap regions that need zero-filled pages
                         *   - Any user VA that hasn't been mapped yet
                         *
                         * The real protection against wild writes is the
                         * vm_map_enter check in sys_mmap (prevents overlapping
                         * mmap regions) and the _coalesce adjacency fix in
                         * malloc (prevents heap metadata from being written
                        ...
                        if (new_page != 0) {
                            ...
                            uint64_t pte_flags = PTE_USER_RW;
